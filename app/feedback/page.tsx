...
export default function Feedback() {
  ...
  const generateFeedback = async (data: InterviewData) => {
    try {
      ...
      // Use a direct API call instead of generateText with model/provider
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          message: `Analyze this Life Design Interview transcript between a student and a ${data.profession} named ${data.interviewee}:
...
ratings (object with overall, questionDepth, activeListening, followUp, etiquette, explorationValue properties as numbers 1-10)`,
      ...
      const text = responseData.response || 'No feedback available.';
      
      try {
        const parsedFeedback = JSON.parse(text) as FeedbackData;
        
        // Add interview statistics
        parsedFeedback.statistics = {
  ...
  const generateFullText = () => {
    if (!interviewData || !feedback) return "";
    ...
    const feedbackText = 
      ...
      `- Opening: ${feedback.interviewStructure.opening}\n` +
      `- Body: ${feedback.interviewStructure.body}\n` +
      `- Closing: ${feedback.interviewStructure.closing}\n\n` +
      `Career Exploration Value: ${feedback.careerExplorationValue}\n\n` +
      `Recommendations:\n${feedback.recommendations.map(r => `- ${r}`).join("\n")}\n\n` +
      `Missed Opportunities:\n${feedback.missedOpportunities.map(o => `- ${o}`).join("\n")}\n\n` +
      `Skills Demonstrated:\n${(feedback.skillsDemonstrated || []).map(s => `- ${s}`).join("\n")}\n\n` +
      `Key Insights Gained:\n${feedback.keyInsights.map(i => `- ${i}`).join("\n")}`;
  ...
  const shareViaEmail = async () => {
    ...
    try {
      ...
      // Create a simple email message template instead of using AI generation
      const emailText = `Dear Recipient,

I'm pleased to share feedback from my recent Life Design Interview with a ${interviewData.profession}.

The feedback includes an analysis of my interviewing skills, highlighting strengths, areas for improvement, and practical recommendations for future interviews.

I've attached the full transcript and detailed feedback for your review.
...
Student`;
      ...
      console.log(`Generated email content: ${emailText}`);
              ...
              <div className="space-y-6">
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      {feedback.ratings && Object.entries(feedback.ratings).map(([key, value]) => {
                        const label = {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <Card>
                    ...
                    <CardContent>
                      <ul className="list-disc pl-5 space-y-2">
                        {feedback.effectiveQuestions.map((question, index) => (
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <ul className="list-disc pl-5 space-y-2">
                        {feedback.missedOpportunities.map((opportunity, index) => (
                  ...
                  <Card>
                    ...
                    <CardContent>
                        <div className="flex flex-wrap gap-2">
                          {feedback.skillsDemonstrated.map((skill, index) => (
